...
class TrackedVehDynamicModel(BaseModel):
    ...
    def build(self) -> ModelBundle:
        ...
        tan_beta = track_contact_length * yaw_rate_cmd * yaw_rate_cmd / (
        ...
        F_y = sign(yaw_rate) * yaw_rate * yaw_rate * lateral_resistance * mass * gravity * s0 / track_contact_length
        ...
        M_r = sign(yaw_rate) * lateral_resistance * mass * gravity / track_contact_length * (
            s0 * s0 - (track_contact_length * track_contact_length) / 4.0
        )

        f_pos_x = vel_x * cos(yaw) - vel_y * sin(yaw)
        f_pos_y = vel_x * sin(yaw) + vel_y * cos(yaw)
        f_yaw = yaw_rate

        f_vel_x = (F_l + F_r - mass * a_c * sin_beta - R_l - R_r) / mass
        f_vel_y = (F_y - mass * a_c * cos_beta) / mass
        f_yaw_rate = (M - M_r + mass * a_c * s0 * cos_beta) / inertia

        f_expl = vertcat(f_pos_x, f_pos_y, f_yaw, f_vel_x, f_vel_y, f_yaw_rate)
        f_impl = xdot - f_expl
